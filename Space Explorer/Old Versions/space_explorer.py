...
class spaceship:
    ...
    def move(self):
        ...
        for x in range(x_bound, x_bound + round(self.velocity[0]), direction):
                for y in range(top_bound, top_bound + HEIGHT):
        ...
        if self.velocity[1] < 0:
            ...
            y_bound = bottom_bound
            direction = 1

        for x in range(left_bound, left_bound + WIDTH):
                for y in range(y_bound, y_bound + round(self.velocity[1]), direction):
                    is_planet, star, color, size = planet_gen((x,y),"daosfuh")
                    if is_planet:
                        planet.instances.append(planet((x,y),size,color))
...
def planet_gen(coords, seed):
    ...
    size = data[4]//2
    is_planet = not data[5] and not data[6] and not data[7]%2 and not data[8]%2
